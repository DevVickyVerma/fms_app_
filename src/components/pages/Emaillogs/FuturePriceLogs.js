...
const FuturePriceLogs = (props) => {
  ...
  const isFuelPriceCancelPermissionAvailable =
    UserPermissions?.includes("fuel-price-cancel");
  const isFuelPricePermissionAvailable =
    UserPermissions?.includes("fuel-price-update");
  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <div
          ...
          onClick={() => handleErrorModal(row)}
          style={{ backgroundColor: row?.expired }}
        >
...

    {
      ...
      cell: (row) => (
        <div
          className="d-flex future-back-color  align-items-center"
          style={{ cursor: "default", background: row?.expired }}
        >
          <div className="ms-2 mt-0 mt-sm-2 d-block">
      ...
      cell: (row) => (
        ...
        >
          <OverlayTrigger placement="top" overlay={<Tooltip>Status</Tooltip>}>
            {row.status === 1 ? (
              <button className="btn btn-success btn-sm">Sucess</button>
            ) : row.status === 0 ? (
    ...
    isFuelPriceCancelPermissionAvailable
      ? {
          ...
          cell: (row) => (
            <div
              className="d-flex future-back-color"
              style={{ background: row?.expired }}
            >
              <div className="ms-2 mt-0 mt-sm-2 d-block">
  ...
  const handleSubmit1 = async (filters) => {
    if (filters) {
      ...
      const formattedStartDate = moment(filters?.range_start_date).format(
        "DD-MM-YYYY"
      );
      const formattedEndDate = moment(filters?.range_end_date).format(
        "DD-MM-YYYY"
      );
          ...
          <Col lg={12}>
            <Card>
              ...
              <Card.Body>
                {data?.length > 0 ? (
                  <>
                    <div className="table-responsive deleted-table mobile-first-table future-price-log-table">
                      <DataTable
                        columns={columns}
                        data={data}
                        noHeader={true}
