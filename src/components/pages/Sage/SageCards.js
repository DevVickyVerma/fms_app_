...
const SageDeduction = (props) => {
  ...
  const handleSubmit = async (values) => {
    if (formik.values.company_id === "" && formik.values.department_id === "") {
      ...
      ErrorAlert("Both Company and Card items are empty!");
    } else {
      try {
        const response = await getData(
          `sage/card-group/heads?company_id=${values.company_id}&card_id=${values.department_id}`
        );
  ...
  const isUpdatePermissionAvailable = permissionsArray?.includes("cardgroup-update");
  ...
  const GetDepartmentList = async (values) => {
    try {
      if (values) {
        const response = await getData(`sage/card-group/list?company_id=${values}`);

        if (response) {

          setDepartmentList(response?.data?.data?.cardGroups);
        } else {
          throw new Error("No data available in the response");
        }
      } else {
  ...
  const DeleteClient = async (formData, index) => {
    try {
      const response = await postData("sage/card-group/head-delete", formData);
  ...
  const combinedOnSubmit = async () => {
    ...
    try {
      ...
      formData.append("card_id", formik.values.department_id);

      const postDataUrl = "/sage/card-group/head-update";
