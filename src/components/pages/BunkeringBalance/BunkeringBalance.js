...
const BunkeringBalance = ({ isLoading, getData, postData }) => {
  ...
  const isAddPermissionAvailable =
    permissionsArray?.includes("bunkering-create");
  const isDeletePermissionAvailable =
    permissionsArray?.includes("bunkering-delete");
  const isEditPermissionAvailable =
    permissionsArray?.includes("bunkering-edit");
  const { id } = useParams();
  ...
  const fetchBunkeringBalanceList = async () => {
    try {
      const response = await getData(
        `/site/bunkering-balance/list?site_id=${id}&page=${currentPage}`
      );
