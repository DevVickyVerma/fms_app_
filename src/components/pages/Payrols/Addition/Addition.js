...
const Addition = ({ isLoading, getData, postData, apidata }) => {
    ...
    const isEditPermissionAvailable = permissionsArray?.includes("addition-edit");
    const isAddPermissionAvailable = permissionsArray?.includes("addition-create");
    const isDeletePermissionAvailable = permissionsArray?.includes("addition-delete");
    ...
    useEffect(() => {
        if (localStorage.getItem("cardsCompanyId")) {
            afterUpdateListApi();
        }
    }, [currentPage]);

    const isstatusPermissionAvailable =
        permissionsArray?.includes("addition-edit");
    ...
    const handleSubmit1 = async (values) => {
        try {
            const response = await getData(
                `/payroll/addition/list?company_id=${values.company_id}&page=${currentPage}`
            );
    ...
    const afterUpdateListApi = async () => {
        let company_id = localStorage.getItem("cardsCompanyId") ? localStorage.getItem("cardsCompanyId") : "";
        try {
            const response = await getData(
                `/payroll/addition/list?company_id=${selectedCompanyId}&page=${currentPage}`
            );
            setData(response?.data?.data?.balances);
            setShowAddButton(true);
            setCount(response.data.data.count);
    ...
    const ToggleStatus = async (formData) => {
        try {
            const response = await postData("/payroll/addition/update-status", formData);
    ...
    const DeleteClient = async (formData) => {
        try {
            const response = await postData("payroll/addition/delete", formData);
            // Console log the response
            if (apidata.api_response === "success") {
                afterUpdateListApi();
            }
                ...
                <span className="text-center d-flex justify-content-center gap-1 flex-wrap">
                    {isEditPermissionAvailable ? (
                        <OverlayTrigger placement="top" overlay={<Tooltip>Edit</Tooltip>}>
                            <Link
                                to={`/update-addition-list/${row.id}`}
...
export default withApi(Addition);
