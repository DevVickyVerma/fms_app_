...
const Holiday = ({ isLoading, getData, postData, apidata }) => {
    ...
    const isEditPermissionAvailable = permissionsArray?.includes("holiday-edit");
    const isAddPermissionAvailable = permissionsArray?.includes("holiday-create");
    const isDeletePermissionAvailable = permissionsArray?.includes("holiday-delete");
    ...
    const isstatusPermissionAvailable =
        permissionsArray?.includes("holiday-edit");
    ...
    const handleSubmit1 = async (values) => {
        try {
            const response = await getData(
                `/payroll/holiday/list?company_id=${values.company_id}&page=${currentPage}`
            );
    ...
    const afterUpdateListApi = async () => {
        try {
            const response = await getData(
                `/payroll/holiday/list?company_id=${selectedCompanyId}&page=${currentPage}`
            );
            setData(response?.data?.data?.balances);
            setShowAddButton(true);
            setCount(response.data.data.count);
    ...
    const ToggleStatus = async (formData) => {
        try {
            const response = await postData("/payroll/holiday/update-status", formData);
    ...
    const DeleteClient = async (formData) => {
        try {
            const response = await postData("payroll/holiday/delete", formData);
            // Console log the response
            if (apidata.api_response === "success") {
                afterUpdateListApi();
            }
...
export default withApi(Holiday);