...
const Designation = ({ isLoading, getData, postData, apidata }) => {
    ...
    const isEditPermissionAvailable = permissionsArray?.includes("designation-edit");
    const isAddPermissionAvailable = permissionsArray?.includes("designation-create");
    const isDeletePermissionAvailable = permissionsArray?.includes("designation-delete");
    ...
    const isstatusPermissionAvailable =
        permissionsArray?.includes("designation-edit");
    ...
    const handleSubmit1 = async (values) => {
        try {
            const response = await getData(
                `/payroll/designation/list?company_id=${values.company_id}&page=${currentPage}`
            );
    ...
    const afterUpdateListApi = async () => {
        try {
            const response = await getData(
                `/payroll/designation/list?company_id=${selectedCompanyId}&page=${currentPage}`
            );
            setData(response?.data?.data?.balances);
            setShowAddButton(true);
            setCount(response.data.data.count);
    ...
    const toggleActiveEdit = (row) => {
        navigate(`/update-designation-list/${row.id}`)
    }
    ...
    const ToggleStatus = async (formData) => {
        try {
            const response = await postData("/payroll/designation/update-status", formData);
    ...
    const DeleteClient = async (formData) => {
        try {
            const response = await postData("payroll/designation/delete", formData);
            // Console log the response
            if (apidata.api_response === "success") {
                afterUpdateListApi();
            }
                ...
                <span className="text-center d-flex justify-content-center gap-1 flex-wrap">
                    {isEditPermissionAvailable ? (
                        <OverlayTrigger placement="top" overlay={<Tooltip>Edit</Tooltip>}>
                            <Link
                                to={`/update-designation-list/${row.id}`}
...
export default withApi(Designation);