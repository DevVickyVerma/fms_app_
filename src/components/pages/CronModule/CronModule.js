...
const CronModule = ({ getData, isLoading }) => {
    ...
    const fetchCronJobApi = async () => {
        try {
            const response = await getData(`/cron-job/logs?cron_job_id=${selectedCronList?.value}`);
            ...
            <Row className=" row-sm">
                <Col lg={12}>
                    <Card>
                        ...
                        <Card.Footer>
                            <button className="btn btn-primary me-2" type="submit" disabled={!selectedCronList} onClick={fetchCronJobApi}>
                                Show Logs
                            </button>
                            ...
                            >
                                {selectedCronList ? (
                                    <Link to={selectedCronList.url} target="_blank" style={{ color: "white" }}>
                                        Submit
                                    </Link>
                                ) : (
                                    <span>Submit</span>
