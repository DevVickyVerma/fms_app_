...
const CardGroup = ({ isLoading, getData }) => {
    ...
    const isEditPermissionAvailable = permissionsArray?.includes("cardgroup-update");
    const isAddPermissionAvailable = permissionsArray?.includes("cardgroup-update");
    ...
    const handleSubmit1 = async (values) => {
        try {
            const response = await getData(
                `/sage/card-group/list?company_id=${values.company_id}`
            );
