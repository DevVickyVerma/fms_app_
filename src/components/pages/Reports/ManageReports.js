...
const ManageReports = (props) => {
  ...
  const isSendReportPermissionAvailable =
    UserPermissions?.includes("send-report");
  ...
  const DownloadReport = async (formValues) => {
    ...
    try {
      ...
      if (
        ...
        (Array.isArray(formik?.values?.selectedSites) &&
          formik?.values?.selectedSites?.length === 0)
      ) {
      ...
      const selectedSiteIds = formik?.values?.selectedSites?.map(
        (site) => site.value
      );
      ...
      link.setAttribute(
        "download",
        `${formValues.reportName}.${fileExtension}`
      );
                    ...
                    <Col lg={4} md={6}>
                      <FormGroup>
                        ...
                        <MultiSelect
                          ...
                          showCheckbox="false"
                        />
                        {formik.touched.selectedSites &&
                          typeof formik.errors.selectedSites === "string" && (
                            <div className="text-danger mt-1">
                              {formik.errors.selectedSites}
                            </div>
                          )}
                      </FormGroup>
                    </Col>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            const selectedReport =
                              ReportList.data?.reports?.find(
                                (item) =>
                                  item.report_code === selectedReportCode
                              );
                            ...
                            formik.setFieldValue(
                              "reportName",
                              selectedReportName
                            ); // Assuming 'reportName' is part of formik initialValues
                          }}
                        >
