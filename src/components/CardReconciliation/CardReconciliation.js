...
const CardReconciliation = (props) => {
  ...
  const validationSchemaForCustomInput = Yup.object({
    ...
    range_start_date: Yup.date().nullable().required("Start Date is required"),
  ...
  const GetDataWithClient = async (values) => {
    ...
    try {
      ...
      try {
        const formattedStartDate = moment(values?.range_start_date).format(
          "YYYY-MM-DD"
        );
        const formattedEndDate = moment(values?.range_end_date).format(
          "YYYY-MM-DD"
        );
        //
        ...
        <div className="d-flex">
          <div className="ms-2 mt-0 mt-sm-2 d-block">
            <h6 className="mb-0 fs-14 fw-semibold">
              {parseFloat(row.total_price_till).toFixed(3)}
            </h6>
          </div>
        </div>
      ),
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-muted fs-15 fw-semibold">
          <OverlayTrigger
            placement="top"
            overlay={<Tooltip>Difference</Tooltip>}
          >
            <span>
  ...
  const handleSubmit = async (files) => {
    ...
    try {
      ...
      const response = await postData(
        "/credit-card/upload-bank-transcation",
        formData
      );
                          ...
                          overlay={
                            <Tooltip>
                              ...
                              {formik.values.uploadedFiles.length > 0 && (
                                <div className="mt-2">
                                  <ul>
                                    {formik.values.uploadedFiles.map(
                                      (file, index) => (
