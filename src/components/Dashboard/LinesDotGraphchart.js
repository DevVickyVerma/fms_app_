...
const LinesDotGraphchart = ({ stockGraphData, showExWatValue = false }) => {
  ...
  const options = {
    ...
    scales: {
      ...
      y1: {
        ...
        ticks: {
          stepSize:
            (Math.max(...trendLineData) - Math.min(...trendLineData)) / 5, // Dynamically adjust step size
          font: {
            weight: "bold",
          },
        },
  ...
  const datasets = stockGraphData?.datasets[selectedFuelType]?.map(
    (dataset) => {
  ...
  return (
    <div>
      <div
        className={`flexspacebetween  pb-4 pt-0 ${showExWatValue ? "d-flex justify-content-between align-items-center" : "text-end"}`}
      >
        {showExWatValue && (
          <>
            <div className=" align-self-start fw-bold">{"Ex. VAT Value"}</div>
          </>
        )}
