...
const CEODashCommonCard = ({
  ...
  <>
    ...
    <Col sm={12} md={4} lg={lg} xl={xl} key={Math.random()}>
      <Card onClick={handleNavigateClick} className="ceo-sats-card-hover uniform-card-height">
        <Card.Body
          className={`text-center ${isParentComponent ? "pointer" : "default-pointer"
            } d-flex flex-column justify-content-between c-p-15`}
        >
          ...
          <div className="flex-grow-1 mt-3">
            ...
            <h2 className="mb-2 number-font c-fs-18">
              {leftSideData ? formatNumber(leftSideData) : "0.0"} {showPPL ? "ppl" : ""}
              {ppl_msg && (
                <OverlayTrigger placement="top" overlay={<Tooltip>{ppl_msg}</Tooltip>}>
                  <i className="fa fa-info-circle pointer ms-1" aria-hidden="true"></i>
                </OverlayTrigger>
              )}
            </h2>
          </div>
          {showRightSide && (
            <div className="mt-3">
              ...
              <h2 className="mb-0 number-font c-fs-18">
                <span className="l-sign">{icon}</span> {RightSideData ? formatNumber(RightSideData) : "0.0"}
              </h2>
            </div>
          )}
          <div className="mt-3">
